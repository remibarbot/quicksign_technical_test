...
from app.models.cnn.training import SimpleTrainer
...
def test_simple_trainer__initializes_with_data_loaders_for_train_and_val(
    ...
    create_fake_image_at_path(
        val_data_path / "image_test.jpg", input_image_size, mode="L"
    )
    create_fake_image_at_path(
        train_data_path / "image_test.jpg", input_image_size, mode="L"
    )
    monkeypatch.setenv("PATH_TO_DATA", str(tmp_path))

    # When
    trainer = SimpleTrainer(input_image_size)
